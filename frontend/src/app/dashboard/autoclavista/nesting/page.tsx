...
import { NestingResponse, nestingApi, NestingPreview, NestingDraft, reportsApi } from '@/lib/api'
import { formatDateIT } from '@/lib/utils'
import { Loader2, RefreshCw, Info, Search, Filter, Download, Plus, AlertCircle, Settings, Layers, Eye } from 'lucide-react'
...
import { TwoLevelNestingModal } from './components/two-level-nesting-modal'
...
export default function NestingPage() {
  ...
  // ✅ NUOVO: Stati per nesting su due piani
  const [twoLevelNestingOpen, setTwoLevelNestingOpen] = useState(false)
  ...
  // ✅ NUOVO: Gestore per il successo del nesting su due piani
  const handleTwoLevelNestingSuccess = () => {
    setTwoLevelNestingOpen(false)
    fetchNestings()
    toast({
      title: 'Nesting su due piani completato',
      description: 'Il nesting su due piani è stato generato con successo.',
    })
  }
  ...
  // 🎯 NUOVE FUNZIONI PER GESTIONE REPORT
  const handleDownloadReport = async (nesting: NestingResponse, e: React.MouseEvent) => {
    e.stopPropagation() // Evita di aprire i dettagli del nesting
    
    try {
      // Prima controlla se il report esiste
      const reportInfo = await reportsApi.checkNestingReport(nesting.id)
      
      if (reportInfo.has_report === false) {
        // Il report non esiste, chiedi se generarlo
        const shouldGenerate = window.confirm(
          `Nessun report disponibile per il nesting #${nesting.id}.\n\nVuoi generare il report ora?`
        )
        
        if (shouldGenerate) {
          await handleGenerateReport(nesting, e)
        }
        return
      }
      
      // Il report esiste, scaricalo
      ...
      link.href = url
      link.download = `report_nesting_${nesting.id}.pdf`
  ...
  const handleGenerateReport = async (nesting: NestingResponse, e: React.MouseEvent) => {
    e.stopPropagation()
    
    try {
      toast({
        title: '🔄 Generazione in corso...',
        description: `Generazione report per nesting #${nesting.id}`,
      })
      
      const result = await reportsApi.generateNestingReport(nesting.id, false)
      
      toast({
        title: '✅ Report generato',
        description: `Report per nesting #${nesting.id} generato con successo`,
      })
      
      // Ora scarica automaticamente il report
      setTimeout(() => {
        handleDownloadReport(nesting, e)
      }, 1000)
      
    } catch (error: any) {
      console.error('Errore nella generazione del report:', error)
      
      let errorMessage = 'Impossibile generare il report.'
      if (error.message) {
        errorMessage = error.message
      }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-start">
        ...
        <div className="flex items-center gap-2">
          ...
          >
            ...
            {/* ✅ NUOVO: Pulsante per nesting su due piani */}
            <Button 
              variant="outline"
              onClick={() => setTwoLevelNestingOpen(true)}
              disabled={isLoading}
              size="sm"
            >
            ...
            {/* ✅ NUOVO: Pulsante per interfaccia visiva */}
            <Button 
              variant="outline"
              onClick={() => window.location.href = '/dashboard/autoclavista/nesting/visual'}
              disabled={isLoading}
              size="sm"
            >
                    ...
                    >
                      ...
                      <TableCell className="text-center">
                        <div className="flex items-center justify-center gap-1">
                          ...
                          />
                          {/* 🎯 NUOVO: Pulsante download report */}
                          <Button 
                            variant="ghost" 
                            size="icon" 
      ...
      {/* ✅ NUOVO: Modal nesting su due piani */}
      <TwoLevelNestingModal
        open={twoLevelNestingOpen}
        onOpenChange={setTwoLevelNestingOpen}
        onSuccess={handleTwoLevelNestingSuccess}
      />
    </div>
  )
} 