...
export default function ProduzionePage() {
  ...
  const fetchData = async () => {
    try {
      ...
      // Carica solo ODL rilevanti per il laminatore (Preparazione e Laminazione)
  ...
  const handleStatusChange = async (odl: ODLResponse, newStatus: "Laminazione" | "Attesa Cura") => {
    setIsAdvancing(true)
    try {
      await odlApi.updateStatusLaminatore(odl.id, newStatus)
        ...
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              ...
              ODL Laminatore ({filteredOdl.length})
            </CardTitle>
          </CardHeader>
          <CardContent>
            {filteredOdl.length === 0 ? (
              ...
              <Table>
                <TableCaption>ODL gestibili dal ruolo Laminatore</TableCaption>
                <TableHeader>
                  <TableRow>
