...
export default function VisualNestingPage() {
  ...
  useEffect(() => {
    if (role && role !== 'Curing' && role !== 'Management') {
      toast({
        variant: 'destructive',
  ...
  useEffect(() => {
    if (role === 'Curing' || role === 'Management') {
      generateNewNesting()
    }
  }, [role])
  ...
  const handleSaveConfig = async () => {
    ...
    try {
      ...
      const newRequest: TwoLevelNestingRequest = {
        ...
        note: configChanges.note || 'Nesting modificato dal Curing'
      }
  ...
  const handleFinalConfirm = async () => {
    ...
    try {
      ...
      await nestingApi.updateStatus(
        ...
        'In sospeso', 
        'Nesting confermato dal Curing tramite interfaccia visiva',
        role || undefined
      )
