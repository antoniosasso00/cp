...
def test_batch_generation(odl_ids):
    ...
    if response.status_code == 200:
        ...
        print(f"   📊 Efficienza: {result.get('efficiency', 0)}%")
        print(f"   📋 Stato iniziale: {result.get('stato', 'unknown')}")
        return batch_id
    else:
...
def test_workflow_transitions(batch_id):
    ...
    # STEP 1: DRAFT → SOSPESO (Conferma)
    print_step(1, "Conferma batch (DRAFT → SOSPESO)")
...
def test_tempo_fase_records(odl_ids):
    ...
    for odl_id in odl_ids:
        ...
        response = requests.get(f"{BASE_URL}/api/tempo-fase?odl_id={odl_id}", 
                              headers=HEADERS)
