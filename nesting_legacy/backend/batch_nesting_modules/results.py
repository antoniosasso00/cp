    ...
    try:
        ...
        if hasattr(main_batch, 'configurazione_json') and main_batch.configurazione_json:
            ...
            if isinstance(config, dict) and 'tool_positions' in config:
                raw_tool_positions = config.get('tool_positions', [])
                if raw_tool_positions:
                    # ✅ ARRICCHISCI CON DATI ODL COMPLETI
                    tool_positions = enrich_tool_positions_with_odl_data(raw_tool_positions, db)
                    logger.info(f"✅ Tool positions trovate e arricchite: {len(tool_positions)} tool")
                else:
        ...
        for batch in related_batches:
            ...
            if hasattr(batch, 'configurazione_json') and batch.configurazione_json:
                if isinstance(batch.configurazione_json, dict):
                    raw_batch_tool_positions = batch.configurazione_json.get('tool_positions', [])
                    # ✅ ARRICCHISCI ANCHE I BATCH CORRELATI
                    if raw_batch_tool_positions:
                        batch_tool_positions = enrich_tool_positions_with_odl_data(raw_batch_tool_positions, db)
